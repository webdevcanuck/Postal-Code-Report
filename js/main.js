...
(function ($, window, document) {
    ...
    $(function () {
        ...
        function postalChop(a, x) {
            c = [];
            for (b = 0; b < a.length; b++) {
                c.push(a[b].substring(0, x)) // strip everything but the first 3 letters
            }
            return c;
        }
        ...
        function b() {
            ...
            mya = inputArr.split(",");
            if ($("select[name=postalchop]").val() > 0) { // if 'by first 3 letters is checked' send to postalChop function
                mya = postalChop(mya, $("select[name=postalchop]").val());
            }
